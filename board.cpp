...
void board::fill(int num){
    for(int i=MAIN_BOARD_HEIGHT - BIAS;i>=MAIN_BOARD_HEIGHT - BIAS-num+1;i--){
        int rand=qrand()%board_w;
        for(int c = 0; c < board_w; c++)
        {
            if(c==rand){
                cells[i * board_w + c].set_color(NO_COLOR);
            }
            else{
                cells[i * board_w + c].set_color(GREY);
            }
        }
    }
}
